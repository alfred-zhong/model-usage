...
```
check-balance.ts        # 通用调度器（缓存 / 域名路由 / Key 查找 / 输出格式化）
lib/
  timeoutFetch.ts       # 共享 HTTP 层：AbortController + 401/429/5xx 分类 + JSON 解析
  runProvider.ts        # 共享 CLI 入口：env 检查 + --json 检测 + 错误 catch
  providers/
    types.ts            # Provider / BalanceResult / ProviderResponse 接口
...
- **余额缓存**路径：`~/.cache/model-usage/balance.json`
  ...
  - 跳过方式：`--force`
  - 写入失败被静默忽略
- **缓存写入时机**：`check-balance.ts` 每次成功调用 `provider.fetchRaw()` 后写入；`raw` 不写入（仅缓存格式化后的展示值）
...
### 共享层（`lib/timeoutFetch.ts`）

- `timeoutFetchJson<T>(url, init)`：10 秒超时 + 非 2xx 抛 `HttpError`（按状态分类）+ JSON 解析失败抛错 + 网络错误原样透传 + AbortError 转 `请求超时 (10 秒)`
- Provider 的 `fetchRaw` 内部调用它，不重复 AbortController 包装
...
- HTTP 请求统一 10 秒超时，统一通过 `lib/timeoutFetch.ts`（不要在 Provider / check-balance 里再加 AbortController）。
